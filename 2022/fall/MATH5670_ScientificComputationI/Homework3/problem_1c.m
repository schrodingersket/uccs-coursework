...
fprintf('-------------------------------------------------------\n')
for i = 1:ntest
    [step_sizes_x(i), step_sizes_y(i), errors(i)] = poisson_5pt(b(1), a(1), b(2), a(2), intervals(i, 1), intervals(i, 2), f, utrue, i == ntest);
...
p = Kp(2);
disp(' ')
fprintf('Least squares fit in x gives E_x(h) = %g * h^%g\n',exp(K),p);
...
K = Kp(1);
p = Kp(2);
fprintf('Least squares fit in y gives E_y(h) = %g * h^%g\n',exp(K),p);
disp(' ')